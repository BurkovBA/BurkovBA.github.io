...
very dissimilar from the input. This phenomenon is called **posterior collapse**.

Some authors claim that this happens when the regularization term of VAE (that moves
prior on $\bf z$ closer to Gaussian) takes over the reconstruction term. Basically, reconstruction term conflicts with
regularization term, and if regularization term wins, input image is ignored by VAE to a significant extent. Hence, one 
might want to introduce some weight (often denoted $\beta$ like regularization strength in ridge regression), which would 
re-balance the impacts of reconstruction and regularization terms in VAE loss.
